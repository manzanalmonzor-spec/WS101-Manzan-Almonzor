...
document.querySelectorAll('a, button, .service-card, .portfolio-item, .skill-tag').forEach(el => {
...
function typeLoop() {
  ...
  if (deleting) {
    ...
    typedEl.textContent = word.substring(0, ++charIdx);
    if (charIdx === word.length) { deleting = true; setTimeout(typeLoop, 1800); }
...
document.getElementById('openContactHero')?.addEventListener('click', openContactModal);
document.getElementById('openContactFooter')?.addEventListener('click', openContactModal);
...
document.querySelectorAll('.service-card').forEach(card => {
  ...
  card.addEventListener('mouseenter', () => {
    card.style.transition = 'transform 0.1s ease, border-color 0.4s ease, background 0.4s ease, box-shadow 0.4s ease';
  });
});
